...
# Ces deux types de conteneurs se distinguent par 
# 1. le comportement lors du passage en argument d'une fonction : le tuple est dupliqué localement dans la fonction, la liste ne l'est pas. Les listent présentent donc l'inconvénient des variables globales : toute modification d'une liste dans une fonction est implicitement répercutée dans le programme principale (avantage tuple)
# 2. les éléments d'un tuple sont protégés : leur modification nécessite la création d'un nouveau conteneur. La rigidité caractéristique des tuple complique (un peu) le codage, mais simplifie (beaucoup) la recherche d'erreurs et l'extension des programme (avantage tuple)
# 3. les méthodes associées aux listes sont plus nombreuses et puissantes que celles des tuples (avantage liste)
